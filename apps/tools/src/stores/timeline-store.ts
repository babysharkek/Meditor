...
export const useTimelineStore = create<TimelineStore>((set, get) => {
  ...
  const updateTracks = (newTracks: TimelineTrack[]) => {
    const tracksWithMain = ensureMainTrack({ tracks: newTracks });
    const sortedTracks = sortTracksByOrder({ tracks: tracksWithMain });
    set({
  ...
  const initialTracks = ensureMainTrack({ tracks: [] });
  const sortedInitialTracks = sortTracksByOrder({ tracks: initialTracks });
  ...
  return {
    ...
    getSortedTracks: () => {
      const { _tracks } = get();
      const tracksWithMain = ensureMainTrack({ tracks: _tracks });
      return sortTracksByOrder({ tracks: tracksWithMain });
    },
    ...
    removeTrackWithRipple: (trackId) => {
      ...
      const updatedTracks = _tracks
        ...
        .map((track) => {
          ...
          const hasOverlaps = checkElementOverlaps({
            elements: updatedElements,
          });
          if (hasOverlaps) {
            const resolvedElements = resolveElementOverlaps({
              elements: updatedElements,
            });
    ...
    updateElementStartTimeWithRipple: (trackId, elementId, newStartTime) => {
      ...
      const updatedTracks = _tracks.map((currentTrack) => {
        ...
        const hasOverlaps = checkElementOverlaps({ elements: updatedElements });
        if (hasOverlaps) {
          const resolvedElements = resolveElementOverlaps({
            elements: updatedElements,
          });
    ...
    loadProjectTimeline: async ({
      ...
      try {
        ...
        if (tracks) {
          ...
          const defaultTracks = ensureMainTrack({ tracks: [] });
